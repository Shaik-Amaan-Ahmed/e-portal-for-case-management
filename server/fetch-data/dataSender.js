...
router.get("/judge-dash-details", async (req, res) => { 
    ...
    try {
        ...
        const reviewCases = judgeDetails.cases.filter((item) => item.status==="Pending for review by judge").length;
        res.status(200).json({message: "success", data: {totalCases: totalCases, activeCases: activeCases,reviewCases: reviewCases}});
